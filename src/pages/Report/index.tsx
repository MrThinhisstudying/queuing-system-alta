...
import {
  getAllReport,
  updateReportsFilter,
} from "../../store/reducers/reportSlice";
...
export const Report = () => {
  ...
  const reportsFilterState = useSelector(
    (state: RootState) => state.report.reportsFilter
  );
  ...
  const pageCount = () => {
    if (reportsFilterState.length === 0)
      return Math.ceil(reportsState.length / PER_PAGE);
      ...
      <div className={styles.body}>
        <div>
          <table>
            ...
            <tbody>
              {reportsFilterState.length === 0 &&
                reportsState.slice(offset, offset + PER_PAGE).map((item) => (
                  <tr key={item.number}>
                    <td>{item.number}</td>
                    <td>{item.name}</td>
                    <td>{item.date}</td>
        ...
        <CSVLink
          data={
            reportsFilterState.length === 0 ? reportsState : reportsFilterState
          }
          headers={headers}
          filename={"report.csv"}
        >
          <div className={styles.dowloadBtn}>
            <div>
              ...
              >
                ...
                />
                <path
                  d="M20.335 10.2779C21.4434 10.2896 22.9834 10.2896 24.3017 10.2896C24.9667 10.2896 25.3167 9.50792 24.85 9.04125C23.17 7.34958 20.16 4.30458 18.4334 2.57792C17.955 2.09959 17.1267 2.42625 17.1267 3.09125V7.16292C17.1267 8.86625 18.5734 10.2779 20.335 10.2779Z"
                  fill="#FF7506"
                />
              </svg>
            </div>
